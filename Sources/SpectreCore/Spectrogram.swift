...
public struct Spectrogram {
    ...
    public init(layout: BinLayout, columnCount: Int, secondsPerColumn: Double,
                values: [Float]) {
        self.layout = layout
        self.columnCount = columnCount
        self.secondsPerColumn = secondsPerColumn
        self.values = values
    }

    public var binCount: Int { layout.binCount }
    public var duration: Double { Double(columnCount) * secondsPerColumn }

    /// Instant décrit par une colonne (son milieu).
    public func time(ofColumn c: Int) -> Double { (Double(c) + 0.5) * secondsPerColumn }

    /// Colonne (fractionnaire) correspondant à un instant.
    public func column(atTime t: Double) -> Double { t / secondsPerColumn - 0.5 }

    public func value(column c: Int, bin i: Int) -> Float {
    ...
    public func averageSpectrum(from t0: Double, to t1: Double) -> [Float] {
    ...
    public static let empty = Spectrogram(layout: BinLayout(), columnCount: 0,
